...
import { PRIVACIDAD_SECCIONES } from "@/lib/politicas-defaults"
...
export default function AvisoPrivacidadPage() {
  ...
  const version     = cfg.legal_privacidad_version || "1.0"
  const fechaUpdate = cfg.legal_privacidad_fecha   || "26 de septiembre de 2025"

  // Contenido dinámico desde BD
  const seccionesDinamicas = secciones.map((s, i) => ({
    ...s,
    contenido: cfg[PRIVACIDAD_SECCIONES[i]?.clave]
      ? cfg[PRIVACIDAD_SECCIONES[i].clave]
      : s.contenido,
  }))
            ...
            <div className="sticky top-10 space-y-8">
              <div className="bg-white dark:bg-gray-800 border border-pink-100 dark:border-gray-700 rounded-[2.5rem] p-8 shadow-sm">
                ...
                <nav className="flex flex-col gap-2">
                  {seccionesDinamicas.map(s => (
                    <a
                      key={s.numero}
                      href={`#seccion-${s.numero}`}
                      className="group flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 hover:text-pink-700 py-2.5 transition-all border-b border-gray-50 last:border-0"
                    >
              ...
              <div className="bg-gradient-to-tr from-pink-600 to-rose-500 rounded-[2.5rem] p-8 text-white shadow-xl shadow-pink-200/50 relative overflow-hidden group">
                <div className="relative z-10">
                  ...
                  <a href="mailto:salonbrennsdudas@gmail.com" className="flex items-center justify-center gap-2 w-full bg-white dark:bg-gray-700 text-pink-700 dark:text-pink-300 py-3.5 rounded-2xl text-xs font-black hover:bg-pink-50 dark:hover:bg-gray-600 transition-colors shadow-lg shadow-black/5">
                    ENVIAR SOLICITUD
                  </a>
                </div>
          ...
          <div className="lg:w-3/4 space-y-12">
            {seccionesDinamicas.map(s => (
              <section
                key={s.numero}
                id={`seccion-${s.numero}`}
                className="scroll-mt-12 bg-white dark:bg-gray-800 rounded-[3rem] border border-pink-50 shadow-sm transition-all duration-500 hover:shadow-md group/section"
              >
                <div className="p-8 md:p-14">
                  <div className="flex items-center gap-6 mb-10">
                    ...
                    <h2 className="relative text-3xl font-black text-gray-900 dark:text-white leading-tight">{s.titulo}</h2>
                  </div>

                  {s.contenido && (
                    <p className="text-gray-600 dark:text-gray-400 text-base leading-relaxed whitespace-pre-line mb-8">{s.contenido}</p>
                  )}

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {s.listas?.map((l, i) => (
                      <div key={i} className="bg-pink-50/40 dark:bg-gray-800/60 border border-pink-100/50 dark:border-gray-700 rounded-[2rem] p-8">
                        ...
                        <ul className="space-y-3">
                          {l.items.map((item, j) => (
                            <li key={j} className="text-sm text-gray-600 dark:text-gray-400 flex gap-3 items-start leading-snug">
                  ...
                  {s.subsecciones?.map((sub, i) => (
                    <div key={i} className="mt-8 bg-gray-50 dark:bg-gray-900/80 rounded-[2.5rem] p-8 md:p-10 border border-gray-100">
                      <div className="flex flex-wrap items-center gap-4 mb-6">
                        <span className="bg-pink-100 text-pink-700 text-[10px] font-black px-4 py-1.5 rounded-full uppercase">APARTADO</span>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-white">{sub.subtitulo}</h3>
                      </div>
                      {sub.intro && <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 italic leading-relaxed">{sub.intro}</p>}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-3">
                        {sub.items.map((item, j) => (
                          <div key={j} className="text-sm text-gray-600 dark:text-gray-400 flex gap-3 items-center py-1">
                      ...
                      {sub.nota && (
                        <div className="mt-8 bg-white/60 rounded-2xl p-5 border-l-4 border-pink-400 shadow-sm">
                          <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed italic">
                            <span className="font-bold text-pink-600 not-italic mr-1">Nota:</span>                            {sub.nota}
                          </p>
                        </div>
                      )}
                    </div>
