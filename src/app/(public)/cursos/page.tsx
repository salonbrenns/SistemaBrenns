          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {cursosPagina.map((curso) => {
              ...
              return (
                <article
                  key={curso.id}
                  className="group bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-sm hover:shadow-2xl border border-pink-50 overflow-hidden transition-all duration-500 hover:-translate-y-2"
                >
                  <Link href={`/curso/${curso.id}`}>
                    <div className="relative h-64 bg-pink-50 overflow-hidden">
                      ...
                      {curso.nivel && (
                        <div className="absolute bottom-4 left-4 bg-white/90 dark:bg-gray-900/80 px-4 py-1.5 rounded-full text-xs font-bold border dark:border-gray-700 dark:text-white">
                          {curso.nivel}
                        </div>
                      )}
