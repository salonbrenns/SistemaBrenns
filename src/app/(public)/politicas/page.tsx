...
import { POLITICAS_SECCIONES } from "@/lib/politicas-defaults"
...
export default function PoliticasPage() {
  ...
  const version     = cfg.legal_politicas_version || "1.0"
  const fechaUpdate = cfg.legal_politicas_fecha   || "26 de septiembre de 2025"

  const seccionesDinamicas = secciones.map((s, i) => {
    const textoBD = cfg[POLITICAS_SECCIONES[i]?.clave]
    if (!textoBD) return s
    return { ...s, contenido: textoBD.split("\n").filter(l => l.trim()) }
  })
    ...
    <main className="min-h-screen bg-[#fffafa] dark:bg-gray-950 scroll-smooth">
      ...
      <div className="relative bg-gradient-to-r from-pink-900 via-pink-800 to-rose-700 py-10 px-6 overflow-hidden border-b border-white/10">
        <div className="absolute inset-0 opacity-5 pointer-events-none">
          <div className="absolute -top-10 -right-10 w-64 h-64 bg-white dark:bg-gray-800 rounded-full blur-3xl" />
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex flex-col gap-2">
              ...
              <h1 className="text-3xl md:text-4xl font-black text-white leading-tight tracking-tight">
                Nuestras <span className="text-pink-300">Politicas</span>
              </h1>
            </div>
            <div className="flex items-center gap-4 bg-black/20 px-5 py-3 rounded-2xl border border-white/5 backdrop-blur-sm">
              <div className="text-right border-r border-white/10 pr-4">
                ...
                <p className="text-pink-400 text-[8px] font-black uppercase tracking-widest">Version</p>
              </div>
              <div className="text-left">
                <p className="text-pink-100 text-[10px] font-medium leading-none mb-1">{fechaUpdate}</p>
                <p className="text-pink-400 text-[8px] font-black uppercase tracking-widest">Ultima actualizacion</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <aside className="lg:w-1/4">
            <div className="sticky top-12 space-y-8">
              <div className="bg-white dark:bg-gray-800 border border-pink-100 dark:border-gray-700 rounded-[2.5rem] p-8 shadow-sm">
                <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mb-6">Categorias</p>
                <nav className="flex flex-col gap-2">
                  {seccionesDinamicas.map((s) => (
                    <a key={s.id} href={`#${s.id}`}
                      className="group flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 hover:text-pink-700 py-3 transition-all border-b border-gray-50 last:border-0">
              ...
              <div className="bg-gradient-to-br from-gray-900 to-pink-900 rounded-[2.5rem] p-8 text-white shadow-xl">
                ...
                <p className="text-xs text-gray-400 mb-6 leading-relaxed">Para cualquier aclaracion sobre reembolsos o envios, contactanos.</p>
                <a href="mailto:salonbrennsdudas@gmail.com"
                  className="block w-full text-center bg-white text-gray-900 dark:text-white py-3 rounded-2xl text-[10px] font-black hover:bg-pink-100 transition-colors uppercase tracking-widest">
                  Escribir a Soporte
                </a>
              </div>
            </div>
              ...
              return (
                <section key={s.id} id={s.id}
                  className={`scroll-mt-12 rounded-[3rem] border shadow-sm transition-all duration-500 hover:shadow-md overflow-hidden ${
                    s.destacado ? "bg-amber-50/50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800" : "bg-white dark:bg-gray-800 border-pink-50 dark:border-gray-700"
                  }`}>
                  <div className="p-10 md:p-14">
                    <div className="flex flex-col md:flex-row md:items-center gap-6 mb-10">
                      ...
                      <div>
                        <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">{s.titulo}</h2>
                        ...
                        {s.destacado && (
                          <div className="inline-flex items-center gap-1.5 bg-amber-200 text-amber-800 text-[10px] font-black px-3 py-1 rounded-full mt-2">
                            <AlertCircle className="w-3 h-3" /> ATENCION AL CLIENTE
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                      {s.contenido.map((parrafo, idx) => (
                        <div key={idx} className="flex gap-4">
                          ...
                          <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{parrafo}</p>
                        </div>
                      ))}
                    </div>
                    {s.footerLink && (
                      <div className="mt-10 pt-8 border-t border-pink-50">
                        <Link href={s.footerLink.href}
                          className="text-xs font-black text-pink-600 hover:text-pink-800 flex items-center gap-2 group">
                          {s.footerLink.text}
