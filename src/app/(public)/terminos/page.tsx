...
import { TERMINOS_SECCIONES } from "@/lib/politicas-defaults"
...
export default function TerminosPage() {
  ...
  // Versión y fecha dinámicas
  const version     = cfg.legal_terminos_version || "1.0"
  const fechaUpdate = cfg.legal_terminos_fecha   || "26 de septiembre de 2025"

  // Contenido dinámico: usar valor de BD si existe, si no el default del array
  const seccionesDinamicas = secciones.map((s, i) => ({
    ...s,
    contenido: cfg[TERMINOS_SECCIONES[i]?.clave] || s.contenido,
  }))
                  ...
                  {seccionesDinamicas.map((s, i) => (
                    ...
                    >
                      <span className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-gray-600 dark:text-gray-400 group-hover:text-pink-700">{s.titulo}</span>
                      </span>
              ...
              <section
                ...
                className={`scroll-mt-12 rounded-[3rem] border transition-all duration-500 ${
                  s.destacado
                    ...
                    : "bg-white dark:bg-gray-800 border-pink-50 dark:border-gray-700 shadow-sm"
                }`}
              >
                <div className="p-10 md:p-14">
                  <div className="flex items-start justify-between mb-8">
                    <div>
                      ...
                      <h2 className="text-3xl font-black text-gray-900 dark:text-white leading-none">{s.titulo}</h2>
                    </div>
                    {s.destacado && (
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-lg leading-relaxed mb-8">{s.contenido}</p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {s.detalles && (
                      <div className="col-span-full bg-white dark:bg-gray-800 border border-pink-100 dark:border-gray-700 rounded-3xl p-8 space-y-3">
                        {s.detalles.map((d, idx) => (
                          <div key={idx} className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    {s.puntos && s.puntos.map((p, idx) => (
                      <div key={idx} className="bg-pink-50/50 dark:bg-gray-800/60 rounded-2xl p-6 border border-pink-100/50 dark:border-gray-700 flex items-center gap-4 group">
                        <div className="w-10 h-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm flex items-center justify-center text-pink-600 font-black group-hover:bg-pink-600 group-hover:text-white transition-colors">
                          {idx + 1}
                        </div>
                        <span className="text-sm font-bold text-gray-700 dark:text-gray-300">{p}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {s.destacado && (
                    <div className="mt-8 bg-amber-100/50 border-l-4 border-amber-500 p-6 rounded-r-[2rem]">
                      ...
                      <p className="text-sm text-amber-900/70 italic leading-relaxed">
                        Es responsabilidad del cliente conocer las condiciones antes de realizar cualquier compra o agendar un servicio.
                      </p>
                    </div>
                  )}
                </div>
