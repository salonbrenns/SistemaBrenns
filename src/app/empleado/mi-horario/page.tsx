...
export default function MiHorarioPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {esAdmin && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-pink-100 dark:border-gray-700 p-5 shadow-sm">
              ...
              {empleados.map(emp => (
                <button key={emp.id} onClick={() => setEmpleadoSel(emp.id)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-semibold transition-all ${
                    empleadoSel === emp.id ? "bg-pink-600 text-white border-pink-600 shadow-sm" : "bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-200 border-gray-200 dark:border-gray-600 hover:border-pink-300"
                  }`}>
      ...
      {horario && !cargandoHorario && (
        <>
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-pink-100 dark:border-gray-700 p-5 shadow-sm flex items-center gap-4 flex-wrap">
          ...
          {horario.dias.length === 0 ? (
            <div className="text-center py-16 text-gray-400 bg-white dark:bg-gray-800 rounded-2xl border border-pink-100 dark:border-gray-700">
                  ...
                  return (
                    <button key={diaId} onClick={() => setDiaActivo(diaId)}
                      className={`flex-shrink-0 px-5 py-3.5 text-sm font-semibold border-b-2 transition-all flex flex-col items-center gap-1 ${
                        diaActivo === diaId ? "border-pink-600 text-pink-600 bg-pink-50 dark:bg-pink-900/20" : "border-transparent text-gray-500 dark:text-gray-400 hover:text-pink-500 hover:bg-pink-50/40 dark:hover:bg-gray-700"
                      }`}>
                      {dia?.label}
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        diaActivo === diaId ? "bg-pink-600 text-white" : "bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-300"
                      ...
                      return (
                        <button key={diaId} onClick={() => setDiaActivo(diaId)}
                          className={`rounded-xl p-3 text-center border transition-all ${
                            diaActivo === diaId ? "border-pink-400 bg-pink-50 dark:bg-pink-900/20" : "border-gray-100 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 hover:border-pink-200"
                          }`}>
