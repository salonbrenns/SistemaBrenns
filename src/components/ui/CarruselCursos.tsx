...
export default function CarruselCursos({ cursos = [] }: { cursos: Curso[] }) {
  const { idx, total, prev, next, goTo, slice } = useCarrusel(cursos, 3)

  return (
    <section className="bg-pink-50/30 dark:bg-gray-900 py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="flex items-end justify-between mb-10">
          <div>
            <p className="text-pink-600 text-xs font-bold uppercase tracking-widest mb-1">Aprende con nosotras</p>
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Academia Brenn&apos;s</h2>
          </div>
            ...
            return (
              <Link key={c.id} href={`/curso/${c.id}`}
                ...
                <div className="relative h-48 overflow-hidden bg-gray-50">
                  {foto ? (
                    <Image src={foto} alt={c.titulo} fill sizes="(max-width:768px) 100vw, 33vw"
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-bold text-gray-900 dark:text-white mt-1 mb-3 line-clamp-1">{c.titulo}</h3>
                  <div className="flex items-center justify-between">
                    <span className="text-lg font-bold text-pink-600">${Number(c.precio_total).toLocaleString()} MXN</span>
                    <span className="inline-flex items-center gap-1 text-xs text-gray-400 font-medium bg-gray-50 dark:bg-gray-700 px-2 py-1 rounded-md">
                      <Clock className="w-3.5 h-3.5" /> {c.duracion_horas} hrs
                    </span>
                  </div>
                </div>
              </Link>
